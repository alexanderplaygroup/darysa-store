...
export const ProductCard = ({
  ...
  return (
    <Link href="/producto/lejia">
      <div className="border-darysa-gris-350-alt/60 group w-full max-w-full overflow-hidden rounded-lg border">
        {/* Product Image Section */}
        <div className="relative flex aspect-square w-full max-w-full items-center justify-center overflow-hidden">
          <div className="relative h-full w-full transition-transform duration-500 ease-out will-change-transform group-hover:scale-105">
            <AppImage src={image} alt={name} fill sizes="300px" className="object-cover" />
          </div>

          {onToggleFavorite && (
            <button
              className="border-darysa-gris-350-alt/60 absolute top-3 right-3 rounded-full border p-1.5 transition-all duration-200 hover:scale-110 hover:bg-white/80"
              ...
              {onAddToCart && (
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    e.stopPropagation(); // evita que el clic suba al enlace padre
                    onAddToCart();
                  }}
                >
